...
class EngineAppApi:
    ...
    def get_script(self, doc_handle):
        msg = json.dumps({'jsonrpc': '2.0', 'id': 0, 'handle': doc_handle, 'method': 'GetScript',
                          'params': []})
        response = json.loads(
    ...
    def set_script(self, doc_handle, script):
        msg = json.dumps({'jsonrpc': '2.0', 'id': 0, 'handle': doc_handle, 'method': 'SetScript',
                          'params': [script]})
        response = json.loads(
            self.engine_socket.send_call(self.engine_socket, msg))
        try:
    ...
    def create_object(self, doc_handle, q_id='LB01', q_type='ListObject',
                      struct_name='qListObjectDef', ob_struct={}):
    ...
    # You can create multiple states within a Qlik Sense app and apply these states to specific
    # objects within the app.
    ...
    def add_alternate_state(self, doc_handle, state_name):
        msg = json.dumps(
            {'jsonrpc': '2.0', 'id': 0, 'handle': doc_handle, 'method': 'AddAlternateState',
             'params': [state_name]})
        response = json.loads(
            self.engine_socket.send_call(self.engine_socket, msg))
        try:
    ...
    # AddFieldFromExpression method: Adds a field on the fly. !! The expression of a field
    # on the fly is persisted but not its values. !!
    ...
    def check_expression(self, doc_handle, expr_value):
        msg = json.dumps(
            {'jsonrpc': '2.0', 'id': 0, 'handle': doc_handle, 'method': 'CheckExpression',
             'params': [expr_value]})
        response = json.loads(
            self.engine_socket.send_call(self.engine_socket, msg))
        try:
    ...
    # CreateConnection method: Creates a connection. A connection indicates from which data source,
    # the data should be taken. The connection can be: an ODBC connection, OLEDB connection,
    # a custom connection, a folder connection (lib connection), an internet connection,
    # Single Sign-On
    def create_connection(self, doc_handle, connect_name, connect_string, connect_type, user_name,
                          password, mod_date='', meta='', sso_passthrough='LOG_ON_SERVICE_USER'):
        msg = json.dumps(
            {'jsonrpc': '2.0', 'id': 0, 'handle': doc_handle, 'method': 'CreateConnection',
             'params': [
                 {
                     'qName': connect_name,
                     'qMeta': meta,
    ...
    # Parameters:
    # qProp (MANDATORY: send dim_id, dim_title, dim_grouping, dim_field, dim_label,
    # meta_def (optional)
    def create_master_dim(self, doc_handle, dim_id, dim_title, dim_grouping='N', dim_field='',
                          dim_label='', meta_def=''):
        msg = json.dumps({'jsonrpc': '2.0', 'id': 0, 'handle': doc_handle,
                          ...
                          'params': [{
                              ...
                              'qDim': {
                                  ...
                                  'qFieldDefs': [dim_field],
                                  'qFieldLabels': [dim_label]
                              },
                              'qMetaDef': {
                                  'title': meta_def
                              }
    ...
    def destroy_dim(self, doc_handle, dim_id):
        msg = json.dumps(
            {'jsonrpc': '2.0', 'id': 0, 'handle': doc_handle, 'method': 'DestroyDimension',
             'params': [{dim_id}]})
        response = json.loads(
            self.engine_socket.send_call(self.engine_socket, msg))
        try:
    ...
    # DestroyObject method: Removes an app object. The children of the object (if any)
    # are removed as well.
    def destroy_object(self, doc_handle, object_id):
        msg = json.dumps(
            {'jsonrpc': '2.0', 'id': 0, 'handle': doc_handle, 'method': 'DestroyObject',
             'params': [{object_id}]})
        response = json.loads(
            self.engine_socket.send_call(self.engine_socket, msg))
        try:
    ...
    # DestroySessionObject method: Removes a session object. The children of the object (if any)
    # are removed as well.
    ...
    def do_save(self, doc_handle):
        msg = json.dumps({'jsonrpc': '2.0', 'id': 0,
                          'handle': doc_handle, 'method': 'DoSave', 'params': []})
        response = json.loads(
            self.engine_socket.send_call(self.engine_socket, msg))
        try:
            return response['result']
    ...
    def get_lineage(self, doc_handle):
        msg = json.dumps({'jsonrpc': '2.0', 'id': 0,
                          'handle': doc_handle, 'method': 'GetLineage', 'params': []})
        response = json.loads(
            self.engine_socket.send_call(self.engine_socket, msg))
        try:
            return response['result']
