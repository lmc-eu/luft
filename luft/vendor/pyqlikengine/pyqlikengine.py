from luft.vendor.pyqlikengine import (engine_app_api, engine_communicator,
                                      engine_field_api, engine_generic_object_api, engine_global_api,
                                      structs)
...
class QixEngine:
    ...
    def __init__(self, url, user_directory, user_id, ca_certs, certfile, keyfile, app_id=None):
        ...
        self.conn = engine_communicator.SecureEngineCommunicator(
            url, user_directory, user_id, ca_certs, certfile, keyfile, app_id)
    ...
    # return a list of tuples where first value in tuple is the actual data value and the second
    # tuple value is that values selection state
    def get_list_object_data(self, dimension_name):
        lb_field = self.eaa.get_field(self.app_handle, dimension_name)
        fld_handle = self.ega.get_handle(lb_field['qReturn'])
        nx_page = self.Structs.nx_page(
            0, 0, self.efa.get_cardinal(fld_handle)['qReturn'])
        lb_def = self.Structs.list_object_def(
            '$', '', [dimension_name], None, None, [nx_page])
        lb_param = {'qInfo': {'qId': 'SLB01',
                              'qType': 'ListObject'}, 'qListObjectDef': lb_def}
        listobj_handle = self.eaa.create_session_object(
            self.app_handle, lb_param)['qReturn']['qHandle']
