class Structs:
    ...
    def nx_hypercube_dimensions(dim_defs):
        dims = []
        dim_defs = dim_defs or []
        for dim_def in dim_defs:
            dims.append({'qLibraryId': '',
    ...
    @staticmethod
    def nx_hypercube_measure_ids(sort_by={}, nx_measures_ids=[]):
        qMeasures = []
        for nx_measures_id in nx_measures_ids:
            qMeasures.append({'qLibraryId': nx_measures_id,
                              'qSortBy': sort_by,
                              })
        return qMeasures
